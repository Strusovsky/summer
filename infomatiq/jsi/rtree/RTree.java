 ...
 * <ul>
 * <li>Fast intersection query performance. To achieve this, the RTree uses only main memory to store entries. Obviously this will only improve performance if
 ...
 *
 * <p>
 * The main reason for the high speed of this RTree implementation is the avoidance of the creation of unnecessary objects, mainly achieved by using primitive
 * collections from the trove4j library.</p>
 */
public class RTree implements SpatialIndex {
     ...
     * <ul>
     * <li>MaxNodeEntries</li> This specifies the maximum number of entries in a node. The default value is 10, which is used if the property is not specified,
     * or is less than 2.
     * <li>MinNodeEntries</li> This specifies the minimum number of entries in a node. The default value is half of the MaxNodeEntries value (rounded down),
     * which is used if the property is not specified or is less than 1.
     * </ul></p>
     *
   ...
   // FindLeaf algorithm inlined here. Note the "official" algorithm 
    ...
    public void nearestN(Point p, int[] result, float furthestDistance) {
        createNearestNDistanceQueue(p, result.length, furthestDistance);
        ...
        while (distanceQueue.size() > 0) {
            if (ii < result.length) {
                result[ii] = distanceQueue.getValue();
            }
            distanceQueue.pop();
            ii++;
        }
        //for (int ii = 0; ii < distanceQueue.size(); ii++) {
        //  result[ii] = distanceQueue.getValue(ii);
        //}
        //distanceQueue.clear();
    }
     ...
     *
     * [Determine cost of putting each entry in each group] For each entry not yet in a group, calculate the area increase required in the covering rectangles
     * of each group
     */
    ...
    /**
     * Recursively searches the tree for the nearest entry. Other queries call execute() on an IntProcedure when a matching entry is found; however nearest()
     * must store the entry Ids as it searches the tree, in case a nearer entry is found. Uses the member variable nearestIds to store the nearest entry IDs.
     *
    ...
    /**
     * Recursively searches the tree for all intersecting entries. Immediately calls execute() on the passed IntProcedure when a matching entry is found.
     *
